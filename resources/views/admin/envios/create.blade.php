...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
    <h2 style="font-family:'Bebas Neue', sans-serif; font-size:32px; letter-spacing:3px;">Registrar Envío</h2>
    ...
    <form action="{{ route('envios.store') }}" method="POST">
       ...
       value="{{ old('fecha_envio', $envio->fecha_envio ?? '') }}">
        ...
        <div class="sl-form-group">
            ...
            <input type="text" name="direccion" class="sl-input" value="{{ old('direccion') }}" required>
        </div>
        <div class="sl-form-group">
            <label class="sl-label">Ciudad</label>
            <input type="text" name="ciudad" class="sl-input" value="{{ old('ciudad') }}" required>
        </div>
        <div class="sl-form-group">
